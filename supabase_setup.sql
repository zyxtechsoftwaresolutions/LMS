-- ============================================
-- LearnHub LMS - Complete Supabase Setup
-- ============================================
-- This file contains all SQL queries needed to set up
-- a new Supabase database for the LearnHub LMS application.
-- 
-- Instructions:
...
-- Students: NAME, REGNO, PHONENO, EMAIL, YEAR, SECTION, DEPT
-- Faculty: NAME, FACULTY_ID, PHONENO, EMAIL, YEAR, SECTION, DEPT
CREATE TABLE public.profiles (
  id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  full_name TEXT NOT NULL, -- NAME (required)
  avatar_url TEXT,
  phone TEXT, -- PHONENO
  email TEXT, -- EMAIL (synced from auth.users)
  regno TEXT, -- Registration number (for students only)
  faculty_id TEXT, -- Faculty ID (for faculty only)
  year TEXT, -- Academic year (e.g., "1st Year", "2nd Year", "3rd Year", "4th Year")
  section TEXT, -- Section (e.g., "A", "B", "C")
  dept TEXT, -- Department (e.g., "CSE", "ECE", "ME", "CE", "EEE")
...
CREATE TABLE public.courses (
  ...
  visibility TEXT CHECK (visibility IN ('public', 'private', 'targeted')) DEFAULT 'public',
  tags TEXT[] DEFAULT '{}',
  video_url TEXT, -- Main course video URL
  materials JSONB DEFAULT '[]', -- Array of materials: [{type: 'pdf', url: '...', title: '...'}, ...]
  links JSONB DEFAULT '[]', -- Array of links: [{url: '...', title: '...'}, ...]
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Course target students table (for targeted visibility)
CREATE TABLE public.course_target_students (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_course_target_students_course ON public.course_target_students(course_id);
CREATE INDEX idx_course_target_students_student ON public.course_target_students(student_id);
...
CREATE INDEX idx_profiles_regno ON public.profiles(regno) WHERE regno IS NOT NULL;
CREATE INDEX idx_profiles_faculty_id ON public.profiles(faculty_id) WHERE faculty_id IS NOT NULL;
CREATE INDEX idx_profiles_dept ON public.profiles(dept);
CREATE INDEX idx_profiles_year ON public.profiles(year);
CREATE INDEX idx_profiles_section ON public.profiles(section);
...
-- Handle new user signup - automatically creates profile and assigns student role
...
-- Course Target Students Policies
...
-- Modules Policies
DROP POLICY IF EXISTS "Modules viewable with course access" ON public.modules;
  ...
  USING (
    EXISTS (
      ...
      AND (
        ...
        (visibility = 'targeted' AND EXISTS (
          SELECT 1 FROM public.course_target_students 
          WHERE course_id = courses.id AND student_id = auth.uid()
        ))
      )
...
-- Lessons Policies
DROP POLICY IF EXISTS "Lessons viewable with enrollment" ON public.lessons;
CREATE POLICY "Lessons viewable with enrollment" 
  ...
  USING (
    EXISTS (
      ...
      WHERE m.id = module_id AND (
        ...
        (c.visibility = 'targeted' AND EXISTS (
          SELECT 1 FROM public.course_target_students 
...
DROP POLICY IF EXISTS "Students can enroll in public courses" ON public.enrollments;
...
DROP POLICY IF EXISTS "Instructors can view student progress" ON public.lesson_progress;
...
-- Quizzes Policies
DROP POLICY IF EXISTS "Published quizzes viewable by enrolled students" ON public.quizzes;
CREATE POLICY "Published quizzes viewable by enrolled students" 
...
CREATE POLICY "Faculty can manage quizzes" 
...
-- Questions Policies
DROP POLICY IF EXISTS "Questions viewable with quiz access" ON public.questions;
CREATE POLICY "Questions viewable with quiz access" 
  ...
  USING (
    EXISTS (
      ...
      WHERE id = quiz_id 
      AND (is_published = true OR created_by = auth.uid() OR public.has_role(auth.uid(), 'admin'))
    )
  );
...
-- Options Policies
DROP POLICY IF EXISTS "Options viewable with question access" ON public.options;
CREATE POLICY "Options viewable with question access" 
  ...
  USING (
    EXISTS (
      ...
      AND (qz.is_published = true OR qz.created_by = auth.uid() OR public.has_role(auth.uid(), 'admin'))
    )
  );

DROP POLICY IF EXISTS "Faculty can manage options" ON public.options;
CREATE POLICY "Faculty can manage options" 
  ...
  USING (
    EXISTS (
      ...
      JOIN public.quizzes qz ON qz.id = q.quiz_id
      WHERE q.id = question_id 
      AND (qz.created_by = auth.uid() OR public.has_role(auth.uid(), 'admin'))
    )
  );
...
DROP POLICY IF EXISTS "Instructors can view course attempts" ON public.quiz_attempts;
...
DROP POLICY IF EXISTS "Users can manage own responses" ON public.question_responses;
...
DROP POLICY IF EXISTS "Instructors can view responses" ON public.question_responses;
CREATE POLICY "Instructors can view responses" 
...
-- Course Target Students Policies (duplicate removed - already defined above)
...
DROP POLICY IF EXISTS "Settings viewable by admins" ON public.site_settings;
...
-- ============================================
-- STEP 8: Optional - Insert Sample Data
-- ============================================
...
INSERT INTO public.categories (name, slug, description) VALUES
  ...
  ('Design', 'design', 'Create beautiful user interfaces and experiences'),
  ('Business', 'business', 'Develop business and entrepreneurship skills');
*/
...
--    - Sign up through your app (creates student by default)
--    - Run this SQL to promote to admin:
